...
class Cool3DGraphics(ThreeDScene):
    ...
    def construct(self):
        ...
        # Commented out axis labels as they might cause issues in 3D rendering
        ...
        # Introduce the axes and the shape (Simplified)
        # Removed Create(axis_labels) and scale=0.2 from FadeIn(shape) for stability
        self.play(Create(axes), FadeIn(shape), run_time=2)
        self.wait(0.5)
        ...
        self.play(
            Rotate(
                ...
                run_time=5,
                rate_func=linear # Moved inside Rotate
            )
        )
